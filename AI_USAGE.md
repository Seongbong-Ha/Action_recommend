...
## 3. AI 결과물을 비판적으로 검토하고 직접 수정한 주요 판단 사례
...
### 💡 사례 9 — Gemini `response_schema` 적용: 모델 레벨 JSON 스키마 강제
...
* **수정 이유**: make run이 중단되는 버그는 평가자가 파이프라인을 전혀 실행할 수 없게 하는 치명적 결함이었고, _match_utterance_id의 NULL 반환은 source_utterance_id 필드 품질을 실질적으로 저해하기 때문입니다.
...
* **AI가 발견·수정한 버그 6건**:
  ...
  ③ `_upsert_action_items` ON CONFLICT 절이 `source_utterance_id`를 무조건 덮어씀 → 재추출 시 기존 유효 링크가 NULL로 파괴 → **`COALESCE(EXCLUDED.source_utterance_id, raw_action_items.source_utterance_id)` 적용**;
...
### 💡 사례 15 — 제출 전 요구사항 반영도 점검 및 문서 최신화
* **AI 수행 항목**: `과제.md`의 필수 요구사항을 기준으로 README, 코드, dbt 모델, Streamlit 대시보드, 보안 설정, 테스트 실행 결과를 교차 점검했습니다.
* **확인된 최신 상태**: `PATH=.venv/bin:$PATH make run`으로 전체 파이프라인이 완료되었고, `PATH=.venv/bin:$PATH make test`는 dbt 테스트 **20/20 PASS**, `PATH=.venv/bin:$PATH make test-unit`은 단위 테스트 **12/12 PASS**였습니다.
* **직접 수정**: README에 남아 있던 과거 명칭 `action_items_raw`를 실제 테이블명 `raw_action_items`로 정정하고, dbt 테스트 개수(16개 → 20개), 단위 테스트 개수(10개 → 12개), 가상환경 활성화 및 `PATH=.venv/bin:$PATH` 실행 팁을 반영했습니다.
* **수정 이유**: 제출 문서가 코드보다 뒤처지면 평가자가 실행 결과와 문서 설명 사이에서 불필요한 의심을 갖게 됩니다. 구현을 과장하지 않도록 대시보드의 현재 추이 위젯은 날짜별 집계이며, 주차별/광고주별 분석은 확장 포인트로 명시했습니다.
