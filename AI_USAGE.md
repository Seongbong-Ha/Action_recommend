...
### 💡 사례 8 — LLM Few-shot 설계: 광고 도메인 암묡적 R&R 케이스 추가
* **AI 초안**: few-shot 예시가 명시적 R&R 핑퐁 1종만 존재하여 한국어 광고 운영 회의의 다양한 패턴을 커버하지 못했습니다.
* **직접 수정**: ① 직무 맥락 기반 암묡적 담당자 추론, ② 단순 수긍 필터링(빈 배열 반환), ③ 상대적 기한 + `related_campaign` 추출 케이스를 직접 시나리오 설계하여 few-shot을 4종으로 확장했습니다.
* **수정 이유**: 광고 대행사 운영 회의에서 "제가 DA 성과 분석 담당이니까"처럼 직무 맥락으로 R&R이 암묡적으로 결정되는 패턴이 빈번함을 알고 있었습니다. 이 케이스를 few-shot 없이 LLM에 위임하면 할루시네이션으로 잘못된 assignee가 배정되거나 누락될 위험이 있어 직접 설계를 지시했습니다.
...
| **M4. LLM 추출 강화**<br>*(extract.py / database.py / dbt)* | - Few-shot 예시 4종 작성 (R&R 핑퐁·암묡적 담당자·단순 수긍 필터링·상대적 기한)<br>- `related_campaign` 필드 스키마·DDL·upsert·dbt mart 전 계층 반영<br>- Slack 페이로드에 confidence 레벨(🟢/🔴) 및 캠페인 정보 표시 추가 | - **암묡적 R&R 지침 설계**: 직무 맥락 기반 담당자 추론 규칙을 `_NOISE_INSTRUCTIONS`에 명문화 — AI 초안은 명시적 R&R만 처리했으나 한국 광고 조직 회의에서 직무 기반 암묡적 R&R이 빈번함을 인지하고 지침 보완을 직접 지시<br>- **related_campaign null 정책 강제**: 캠페인 미언급 시 강제 추측 금지 원칙을 few-shot 및 지침에 명시 — 없는 캠페인을 억지로 채우면 그룹핑 왜곡이 생김을 판단하여 추가 |
